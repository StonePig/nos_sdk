...
/*定义更新读取回来的对象值的函数指针类型*/
/*更新读回来的线圈状态*/
typedef void (*UpdateCoilStatusType)(uint8_t salveAddress,uint16_t startAddress,uint16_t quantity,bool *stateValue) reentrant;

/*更新读回来的输入状态值*/
typedef void (*UpdateInputStatusType)(uint8_t salveAddress,uint16_t startAddress,uint16_t quantity,bool *stateValue) reentrant;

/*更新读回来的保持寄存器*/
typedef void (*UpdateHoldingRegisterType)(uint8_t salveAddress,uint16_t startAddress,uint16_t quantity,uint16_t *registerValue) reentrant;

/*更新读回来的输入寄存器*/
typedef void (*UpdateInputResgisterType)(uint8_t salveAddress,uint16_t startAddress,uint16_t quantity,uint16_t *registerValue) reentrant;
