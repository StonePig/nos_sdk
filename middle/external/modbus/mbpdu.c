...
uint16_t GenerateReadWriteCommand(ObjAccessInfo objInfo,bool *statusList,uint16_t *registerList,uint8_t *commandBytes)
{
  ...
  if((objInfo.functionCode==WriteSingleRegister)&&(registerList!=NULL))
  {
    ...
    commandBytes[index++]=(*registerList) & 0xFF;
  }
  ...
  if((objInfo.functionCode==WriteMultipleCoil)&&(statusList!=NULL))
  {
    ...
    bytesCount=ConvertBoolArrayToMBByteArray(statusList,objInfo.quantity,commandBytes + index + 1);
    commandBytes[index++]=bytesCount & 0xFF;
    index+=bytesCount;
  ...
  if((objInfo.functionCode==WriteMultipleRegister)&&(registerList!=NULL))
  {
    ...
    commandBytes[index++]=objInfo.quantity & 0xFF;             //数量低字节
    
    bytesCount=ConvertRegisterArrayToMBByteArray(registerList,objInfo.quantity,commandBytes + index + 1);
    commandBytes[index++]=bytesCount & 0xFF;		//字节数量
    index+=bytesCount;
    // for(i=0;i<bytesCount;i++)
    // {
    //   commandBytes[index++]=byteArray[i];
...
uint16_t GenerateMasterAccessRespond(uint8_t *receivedMessage,bool *statusList,uint16_t *registerList,uint8_t *respondBytes)
{
  ...
  if(((functionCode==ReadCoilStatus)||(functionCode==ReadInputStatus))&&(statusList!=NULL))
  {
    ...
    bytesCount=ConvertBoolArrayToMBByteArray(statusList,bitsQuantity,respondBytes + index + 1);
    respondBytes[index++]=bytesCount & 0xFF;//字节数
    index += bytesCount;
    // for(i=0;i<bytesCount;i++)
    // {
  ...
  if(((functionCode==ReadHoldingRegister)||(functionCode==ReadInputRegister))&&(registerList!=NULL))
  {
    ...
    bytesCount=ConvertRegisterArrayToMBByteArray(registerList,registerCount,respondBytes + index + 1);
    respondBytes[index++]=bytesCount & 0xFF;		//字节数量
    index += bytesCount;
    // for(i=0;i<bytesCount;i++)
    // {
...
static uint16_t ConvertRegisterArrayToMBByteArray(uint16_t *sData,uint16_t length,uint8_t * oData)
{
  ...
  if(length>0)
  {
    for(i=0;i<length;i++)
    {
      ...
      oData[returnLength++]=sData[i] & 0xFF;
    }
  }
  return returnLength;
}
