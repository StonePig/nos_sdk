...
extern void WriteMultipleRegisterRespond(uint8_t salveAddress, uint16_t startAddress, uint16_t quantity);


typedef void (*HandleSlaveRespond_t)(RTULocalMasterType *master, uint8_t *receivedMesasage, uint16_t startAddress, uint16_t quantity) reentrant;

static HandleSlaveRespond_t HandleSlaveRespond[] = {HandleReadCoilStatusRespond,
...
void ParsingSlaveRespondMessage(RTULocalMasterType *master, uint8_t *recievedMessage, uint8_t *command)
{
  ...
  {
#if 0  
    if(recievedMessage[1] == WriteMultipleRegister && command)
    {
      //比较前6个字节是否相等
      for(i = 0; i < 6; i++)
      {
        if(recievedMessage[i] != command[i])
          return;
      }
      startAddress = (uint16_t)recievedMessage[2];
      startAddress = (startAddress << 8) + (uint16_t)recievedMessage[3];
      quantity = (uint16_t)recievedMessage[4];
      quantity = (quantity << 8) + (uint16_t)recievedMessage[5];
      WriteMultipleRegisterRespond(recievedMessage[0], startAddress, quantity);
    }
#endif	
    return;
  }
  ...
  /*判断功能码是否有误*/
  fuctionCode = (FunctionCode)recievedMessage[1];
  ...
  /*校验接收到的信息是否有错*/
  byteCount = recievedMessage[2];
  chechMessageNoError = CheckRTUMessageIntegrity(recievedMessage, byteCount + 5);
...
static void HandleReadCoilStatusRespond(RTULocalMasterType *master, uint8_t *receivedMessage, uint16_t startAddress, uint16_t quantity)
{
  ...
  slaveAddress = receivedMessage[0];
