...
typedef uint16_t (*HandleMasterCommand_t)(uint16_t startAddress, uint16_t quantity, uint8_t *receivedMessage, uint8_t *respondBytes) reentrant;

static  HandleMasterCommand_t HandleMasterCommand[] = {HandleReadCoilStatusCommand,
...
uint16_t ParsingMasterAccessCommand(uint8_t *receivedMessage, uint8_t *respondBytes, uint16_t rxLength, uint8_t StationAddress)
{
  ...
  chechMessageNoError = CheckRTUMessageIntegrity(receivedMessage, dataLength);
